...
CREATE POLICY "Authenticated users can insert contacts" ON contacts FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "Users can update own contacts" ON contacts FOR UPDATE USING (created_by = auth.uid());
CREATE POLICY "Admins can update all contacts" ON contacts FOR UPDATE USING (public.is_admin(auth.uid()));
CREATE POLICY "Admins can delete contacts" ON contacts FOR DELETE USING (public.is_admin(auth.uid()));
